export function mapOrderDetails(raw: any) {
  ...
  return {
    ...
    items: (raw.items ?? []).map((item: any, index: number) => ({
      ...
      warehouse_id:
        item.productVariant?.inventory?.[index]?.warehouse.id ?? null,
      warehouse_name:
        item.productVariant?.inventory?.[index]?.warehouse?.name ?? null,
    ...
    address: raw.address
      ? {
          ...
          address_line_1: raw.address.address_line_1, // normalize snake_case
