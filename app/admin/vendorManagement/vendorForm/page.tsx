  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm shadow-gray-100/60 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/60">
        ...
        {subtitle && (
          <p className="text-theme-body-sm text-gray-500 mt-0.5">{subtitle}</p>
        )}
      </div>
...
export function ReviewRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-gray-50 last:border-0">
      <span className="text-theme-caption font-semibold text-gray-400 uppercase tracking-wide shrink-0 w-40">
        {label}
      </span>
      <span className="text-theme-body-sm text-gray-800 font-medium text-right break-all">
    ...
    <div className="border border-gray-100 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <span className="text-theme-body-sm font-bold text-gray-700">
            {title}
          </span>
        </div>
        <button
    ...
    <>
      ...
      <main className="admin_vendorManagement">
        ...
        <header className="flex justify-between items-center my-6">
          <div>
            <h1 className="font-bold text-theme-h4 text-gray-900">
              Register New Vendor
            </h1>
            <p className="text-theme-body-sm text-gray-500 mt-1">
              Complete all steps to create a vendor account from the admin
              panel.
            </p>
          </div>
          <Link
                          ...
                          >
                            {subField.type === "select" ? (
                              <select
                                ...
                                {...register(
                                  subField.id as keyof VendorRegisterSchema,
                                )}
                              >
                          ...
                          <p className="input-error">
                            {
                              (
                                errors[
                                  field.id as keyof VendorRegisterSchema
                                ] as any
                              )?.message
                            }
                          </p>
            ...
            >
              <div className="space-y-4">
                ...
                >
                  ...
                  <ReviewRow
                    label="Owner Name"
                    value={`${snap.store_owner_first_name} ${snap.store_owner_last_name}`}
                  />
                  <ReviewRow label="Business Email" value={snap.email} />
                  <ReviewRow
                    label="Phone"
                    value={`${snap.country_code} ${snap.phone_number}`}
                  />
                ...
                >
                  <ReviewRow
                    ...
                    value={
                      snap.company_domain
                        ? `${snap.company_domain}.platform.com`
                        : ""
                    }
                  />
                </ReviewGroup>
                  ...
                  {currentCountryFields.length > 0 ? (
                    currentCountryFields.map((f) => (
                      <ReviewRow
                        key={f.value}
                        label={f.label}
                        value={complianceValues[f.value] ?? ""}
                      />
                    ))
                  ) : (
                    <div className="py-2.5 text-theme-caption text-gray-400 italic">
                      No compliance fields selected.
                    </div>
                  )}
                </ReviewGroup>
                ...
                >
                  ...
                  {financialFileMap.filter((e) => e.file).length > 0 && (
                    <div className="py-2.5">
                      <p className="text-theme-tiny font-bold text-gray-400 uppercase tracking-wider mb-1.5">
                        Financial
                      </p>
                      {financialFileMap
                        .filter((e) => e.file)
                        .map((e) => (
                          ...
                          >
                            ...
                            />
                            <span className="text-theme-caption text-gray-700 truncate">
                              {e.file!.name}
                            </span>
                            <span className="text-theme-tiny text-gray-400 shrink-0 ml-auto">
                              {e.type}
                            </span>
                          </div>
                        ))}
