...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case SearchPageActionType.SET_POPULAR:
      return { ...state, popularSuggestions: action.payload };
    case SearchPageActionType.SET_LOADING:
...
function SearchPageContent() {
      ...
      try {
        ...
        if (Array.isArray(popular?.data)) {
          const names: string[] = popular.data
            ...
            .filter(Boolean);
          dispatch({
            type: SearchPageActionType.SET_POPULAR,
            payload: Array.from(new Set(names)),
          });
        }
      } catch {
    ...
    return (
      <div className="min-h-screen bg-gray-50/50 flex items-center justify-center">
        <div className="flex items-center gap-3 text-gray-500">
          ...
          <span className="text-theme-body-sm font-medium">
  ...
  // ── Idle screen ───────────────────────────────────────────────────────────
  const pills =
    state.popularSuggestions.length > 0
        ...
        >
          ...
          <div className="rounded-2xl p-8 bg-gradient-to-br from-theme-primary to-theme-primary/80 text-theme-primary-foreground flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-md gap-4">
            <div className="space-y-1.5">
              <h3 className="text-theme-h6 font-extrabold tracking-wide">
