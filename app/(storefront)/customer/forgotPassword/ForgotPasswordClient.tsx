...
export default function ForgotPasswordClient() {
        ...
        if (diff <= 0) {
          ...
          if (!state.isLockedOut) {
            dispatch({
              ...
              payload: FORGOT_PASSWORD_TEXT.ERR_OTP_EXPIRED,
            });
          }
        } else {
  ...
  const handleRequestOtp = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: FORGOT_PASSWORD_TEXT.MSG_OTP_SENT,
      });
      ...
      dispatch({
        type: ForgotPasswordActionType.SET_ERROR,
        payload: FORGOT_PASSWORD_TEXT.ERR_OTP_SEND_FAILED,
      });
    } finally {
  ...
  const handleResetPassword = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      dispatch({
        type: ForgotPasswordActionType.SET_SUCCESS,
        payload: FORGOT_PASSWORD_TEXT.MSG_RESET_SUCCESS,
      });
      ...
      if (newAttempts >= 3) {
        ...
        dispatch({
          type: ForgotPasswordActionType.SET_ERROR,
          payload: FORGOT_PASSWORD_TEXT.ERR_LOCKED_OUT,
        });
      } else {
        dispatch({
          type: ForgotPasswordActionType.SET_ERROR,
          payload:
            err.message ||
            FORGOT_PASSWORD_TEXT.ERR_RESET_FAILED(3 - newAttempts),
        });
      }
    } finally {
      dispatch({ type: ForgotPasswordActionType.SET_LOADING, payload: false });
  ...
  const passwordStrength = (password: string) => {
    ...
    if (strength <= 2)
      return {
        strength,
        label: FORGOT_PASSWORD_TEXT.STRENGTH_WEAK,
        color: "bg-red-500",
      };
    if (strength <= 3)
      return {
        strength,
        label: FORGOT_PASSWORD_TEXT.STRENGTH_FAIR,
        color: "bg-amber-500",
      };
    if (strength <= 4)
      return {
        strength,
        label: FORGOT_PASSWORD_TEXT.STRENGTH_GOOD,
        color: "bg-lime-500",
      };
    return {
      strength,
      label: FORGOT_PASSWORD_TEXT.STRENGTH_STRONG,
        ...
        <div className="bg-gradient-to-br from-theme-primary via-theme-primary/95 to-theme-secondary px-8 py-10 text-theme-primary-foreground relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h2 className="text-theme-h4 font-bold">
                  {FORGOT_PASSWORD_TEXT.TITLE}
                </h2>
                <div className="flex items-center gap-2 mt-1">
                  <div
                    className={`w-8 h-1 rounded-full ${state.step >= 1 ? "bg-theme-primary-foreground" : "bg-theme-primary-foreground/30"}`}
                  ></div>
                  <div
                    className={`w-8 h-1 rounded-full ${state.step >= 2 ? "bg-theme-primary-foreground" : "bg-theme-primary-foreground/30"}`}
                  ></div>
                </div>
              </div>
            </div>
                  ...
                  {state.isLoading ? (
                    ...
                    <>
                      {state.resendCooldown > 0
                        ? FORGOT_PASSWORD_TEXT.BTN_WAIT(state.resendCooldown)
                        : FORGOT_PASSWORD_TEXT.BTN_SEND}
                      {state.resendCooldown === 0 && (
              ...
              >
                ...
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  <div className="flex justify-between items-center mb-1">
                    <p className="text-theme-caption text-slate-500">
                      {FORGOT_PASSWORD_TEXT.LBL_CODE_SENT}
                    </p>
                    {state.otpExpiryTime && (
                      <span
                        className={`text-theme-caption font-bold ${timeLeft === "00:00" ? "text-red-500" : "text-amber-500"}`}
                      >
                        {FORGOT_PASSWORD_TEXT.LBL_EXPIRES}
                        {timeLeft}
                      </span>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    >
                      {state.resendCooldown > 0
                        ? FORGOT_PASSWORD_TEXT.BTN_RESEND_WAIT(
                            state.resendCooldown,
                          )
                        : FORGOT_PASSWORD_TEXT.BTN_RESEND}
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    >
                      {state.showPassword
                        ? FORGOT_PASSWORD_TEXT.BTN_HIDE
                        : FORGOT_PASSWORD_TEXT.BTN_SHOW}
                    </button>
                  </div>
