        ...
        <div className="text-center animate-in fade-in zoom-in duration-300">
          ...
          <div className="bg-red-50 text-red-700 p-4 rounded-lg mb-6 text-theme-body-sm text-left border border-red-100">
            <p className="font-semibold mb-1">
              {AUTH_ERROR_TEXT.ERROR_DETAILS_LABEL}
            </p>
            <p>{errorMessage}</p>
            {status && (
              <p className="mt-1 text-theme-caption text-red-500">
                {AUTH_ERROR_TEXT.STATUS_CODE_LABEL} {status}
              </p>
            )}
            {email && (
              <p className="mt-1 text-theme-caption text-red-500">
                {AUTH_ERROR_TEXT.ACCOUNT_LABEL} {email}
              </p>
            )}
          </div>

          <p className="text-slate-600 mb-8">{AUTH_ERROR_TEXT.DESCRIPTION}</p>
