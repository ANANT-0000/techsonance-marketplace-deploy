...
export type Action =
  ...
  | { type: ActionType.UPDATE_STEP; payload: { index: number; status: Step['status'] } }
...
export default function AdminLoginPage() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result: { status: boolean; message: string; data?: any } =
      await adminLogin({ admin_id: state.adminLoginID!, password: state.adminLoginPass! });
  ...
  const startRedirect = (userId: string) => {
    ...
    countdownRef.current = setInterval(() => {
      elapsed += interval;
      dispatchState({ type: ActionType.SET_REDIRECT_PROGRESS, payload: Math.max(0, 100 - (elapsed / total) * 100) });
      dispatchState({ type: ActionType.SET_COUNTDOWN, payload: Math.ceil((total - elapsed) / 1000) });
      if (elapsed >= total) {
    ...
    <main className="min-h-screen   flex items-center justify-center p-6 font-sans">
      <div className="w-full  bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        ...
        <div className="px-8 pt-8 pb-6 border-b border-gray-100">
          ...
          <h1 className="text-theme-h6 font-semibold text-gray-900 mb-0.5">{ADMIN_LOGIN_TEXT.TITLE}</h1>
          <p className="text-theme-body-sm text-gray-500">{ADMIN_LOGIN_TEXT.SUBTITLE}</p>
        </div>
        ...
        {state.uiState === 'idle' && (
          <form onSubmit={submitHandler} className="w-full px-8 py-6 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-md font-medium text-gray-600 tracking-wide">{ADMIN_LOGIN_TEXT.ID_LABEL}</label>
              <div className="relative">
                ...
                <input
                  ...
                  onChange={e => dispatchState({ type: ActionType.SET_LOGIN_ID, payload: e.target.value })}
                />
              </div>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-md font-medium text-gray-600 tracking-wide">{ADMIN_LOGIN_TEXT.PASS_LABEL}</label>
              <div className="relative">
                <input
                  type={state.showPass ? "text" : "password"}
                  className="w-full pl-8 pr-10 py-2.5 border border-gray-200 rounded-lg text-md"
                  onChange={e => dispatchState({ type: ActionType.SET_LOGIN_PASS, payload: e.target.value })}
                />
                <button
                  type="button"
                  onClick={() => dispatchState({ type: ActionType.TOGGLE_SHOW_PASS })}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition"
                  aria-label={state.showPass ? ADMIN_LOGIN_TEXT.HIDE_PASS : ADMIN_LOGIN_TEXT.SHOW_PASS}
                >
                  {state.showPass
                    ? <EyeOff size={16} />
                    : <Eye size={16} />
        ...
        {state.uiState === 'loading' && (
          <div className="px-8 py-8 flex flex-col items-center">
            <p className="text-theme-caption font-medium text-gray-400 uppercase tracking-widest mb-1">{ADMIN_LOGIN_TEXT.LOADING_TITLE}</p>
            <p className="text-theme-caption text-gray-400 mb-5">{ADMIN_LOGIN_TEXT.LOADING_SUBTITLE}</p>
        ...
        {state.uiState === 'success' && (
          <div className="px-8 py-8 flex flex-col items-center text-center">
            ...
            <h2 className="text-theme-body font-semibold text-gray-900 mb-1">{ADMIN_LOGIN_TEXT.SUCCESS_TITLE}</h2>
            <p className="text-theme-body-sm text-gray-500 mb-5">{ADMIN_LOGIN_TEXT.SUCCESS_SUBTITLE}</p>
            <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden mb-2">
              <div className="h-1.5 bg-sky-500 rounded-full transition-all" style={{ width: `${state.redirectProgress}%` }} />
            </div>
            <p className="text-theme-caption text-gray-400">{ADMIN_LOGIN_TEXT.REDIRECT_PREFIX}{state.countdown}{ADMIN_LOGIN_TEXT.REDIRECT_SUFFIX}</p>
          </div>
        )}
        ...
        {state.uiState === 'error' && (
          <div className="px-8 py-8 flex flex-col items-center text-center">
            ...
            <h2 className="text-theme-body font-semibold text-gray-900 mb-1">{ADMIN_LOGIN_TEXT.ERROR_TITLE}</h2>
            <p className="text-theme-body-sm text-gray-500 mb-5">{state.error || ADMIN_LOGIN_TEXT.ERROR_DEFAULT_MSG}</p>
            <button
