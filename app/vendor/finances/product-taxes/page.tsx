...
const productTaxHeader = [
  PRODUCT_TAX_TEXT.TABLE.HEADERS.PRODUCT_NAME,
  PRODUCT_TAX_TEXT.TABLE.HEADERS.SKU,
  PRODUCT_TAX_TEXT.TABLE.HEADERS.ASSIGNED_TAX_RATE,
  PRODUCT_TAX_TEXT.TABLE.HEADERS.TOTAL_TAX,
  PRODUCT_TAX_TEXT.TABLE.HEADERS.MAPPING_STATUS,
  PRODUCT_TAX_TEXT.TABLE.HEADERS.LAST_UPDATED,
  PRODUCT_TAX_TEXT.TABLE.HEADERS.ACTIONS,
];
      ...
      <header className="flex justify-between items-center my-6">
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 font-semibold text-theme-body-sm bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 rounded-xl px-5 py-2.5 transition-colors shadow-sm">
            <Download size={16} /> {PRODUCT_TAX_TEXT.HEADER.EXPORT}
          </button>
          <button
            className={`flex items-center gap-2 font-semibold text-theme-body-sm rounded-xl px-5 py-2.5 transition-colors shadow-sm ${selectedProductIds.length > 0 ? "bg-blue-500 hover:bg-blue-600 text-white" : "bg-gray-200 text-gray-500 cursor-not-allowed"}`}
            onClick={() => setModalOpen(true)}
            disabled={selectedProductIds.length === 0}
          >
            <Tag size={16} />
            {PRODUCT_TAX_TEXT.HEADER.BULK_ASSIGN}
          </button>
        </div>
      </header>
      <div className="relative flex flex-wrap justify-between rounded-xl items-center py-3 px-4 gap-3 bg-white border border-gray-200 shadow-sm mb-4">
        ...
        <span className="flex flex-1 min-w-[220px] items-center gap-2 border border-gray-200 bg-gray-50 py-2 px-3 rounded-xl focus-within:border-emerald-400 focus-within:bg-white transition-colors">
          ...
          <input
            ...
            placeholder={PRODUCT_TAX_TEXT.FILTERS.SEARCH_PLACEHOLDER}
          />
        </span>
        ...
        <span className="flex flex-wrap gap-3 items-center">
          <select
            ...
            name="status"
          >
            <option value="all">{PRODUCT_TAX_TEXT.FILTERS.ALL_MAPPINGS}</option>
            <option value="cleared">{PRODUCT_TAX_TEXT.FILTERS.ASSIGNED}</option>
            <option value="pending">
              {PRODUCT_TAX_TEXT.FILTERS.UNASSIGNED}
            </option>
          </select>
          ...
          >
            <option value="date_newest">
              {PRODUCT_TAX_TEXT.FILTERS.SORT_NEWEST}
            </option>
            <option value="date_oldest">
              {PRODUCT_TAX_TEXT.FILTERS.SORT_OLDEST}
            </option>
            <option value="amount_highest">
              {PRODUCT_TAX_TEXT.FILTERS.SORT_HIGHEST}
            </option>
          </select>
          ...
          {isOpen ? (
            ...
            >
              {date
                ? date.toDateString()
                : PRODUCT_TAX_TEXT.FILTERS.SELECT_DATE}
              <ChevronUp size={16} />
            </button>
          ) : (
            <button
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr>
                ...
                >
                  {PRODUCT_TAX_TEXT.TABLE.NO_DATA}
                </td>
              </tr>
            ) : (
                ...
                >
                  ...
                  <td className="p-4">
                    <Link
                      ...
                      className="font-semibold text-blue-600 hover:underline line-clamp-2 text-theme-body-sm text-wrap max-w-24"
                    >
                      {item.product_name}
                    </Link>
                  </td>

                  <td className="p-4 text-gray-500 text-theme-body-sm font-mono">
                    {item.sku}
                  </td>

                  <td className="p-4 text-gray-700 text-theme-body-sm font-medium">
                    {item.tax_slabs || "—"}
                  </td>
                  ...
                  <td className="p-4">
                    {item.is_mapped ? (
                      <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200 py-1 px-3 rounded-full text-theme-caption font-semibold">
                        ...
                        {PRODUCT_TAX_TEXT.TABLE.STATUS_ASSIGNED}
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 bg-red-50 text-red-700 border border-red-200 py-1 px-3 rounded-full text-theme-caption font-semibold">
                        {PRODUCT_TAX_TEXT.TABLE.STATUS_MISSING}
                      </span>
                    )}
                  </td>
                  ...
                  <td className="p-4">
                    ...
                    >
                      {item.is_mapped
                        ? PRODUCT_TAX_TEXT.TABLE.ACTION_CHANGE
                        : PRODUCT_TAX_TEXT.TABLE.ACTION_ASSIGN}
                    </button>
                  </td>
                </tr>
              ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleAssignTax} className="p-6">
              <label className="text-theme-body-sm font-semibold text-gray-700 block mb-2">
                {PRODUCT_TAX_TEXT.MODAL.LABEL}
              </label>
              <select
                required
              ...
              >
                <option value="" disabled>
                  {PRODUCT_TAX_TEXT.MODAL.SELECT_PLACEHOLDER}
                </option>
              ...
              {availableRates.length === 0 && (
                <p className="text-theme-caption text-amber-600 mt-2">
                  {PRODUCT_TAX_TEXT.MODAL.NO_RATES_WARNING}{" "}
                  <Link
                    href={`/vendor/finances/tax-rates/new`}
                    className="underline font-bold"
                  >
                    {PRODUCT_TAX_TEXT.MODAL.CREATE_LINK}
                  </Link>
                </p>
              )}
              ...
              <div className="flex justify-end gap-3 mt-8">
                <button
                  type="button"
                  onClick={() => setModalOpen(false)}
                  className="px-5 py-2.5 text-theme-body-sm font-semibold text-gray-600 hover:bg-gray-100 rounded-xl transition-colors"
                >
                  {PRODUCT_TAX_TEXT.MODAL.CANCEL}
                </button>
                <button
                  disabled={saving || availableRates.length === 0}
                  type="submit"
                  className="px-5 py-2.5 text-theme-body-sm font-semibold text-white bg-blue-500 hover:bg-blue-600 rounded-xl shadow-sm transition-colors disabled:opacity-50"
                >
                  {saving
                    ? PRODUCT_TAX_TEXT.MODAL.APPLYING
                    : PRODUCT_TAX_TEXT.MODAL.APPLY}
                </button>
              </div>
            </form>
          </div>
