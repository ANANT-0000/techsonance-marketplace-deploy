...
const METHOD_ICONS: Record<string, React.ReactNode> = {
  [SELECTED_PAYMENT_METHOD_TEXT.UPI]: (
  ...
  return (
    <motion.label
      ...
      className={`flex items-start gap-3 p-3.5 border rounded-xl cursor-pointer transition-all duration-150 ${
        isSelected
          ? "border-theme-primary bg-theme-primary/10 shadow-sm shadow-theme-primary/10"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <div
            className={`w-6 h-6 rounded-md border flex items-center justify-center shrink-0 ${iconBg}`}
          >
            {icon}
          </div>
          <span
            className={`text-theme-caption-lg font-semibold ${isSelected ? "text-gray-900" : "text-gray-700"}`}
          >
            {method}
          </span>
        </div>
