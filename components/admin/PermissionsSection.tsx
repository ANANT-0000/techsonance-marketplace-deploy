...
export default async function PermissionsSection({ permissions, adminId }: Props) {
  ...
  return (
    <div>
      <h2 className="text-sm font-semibold text-gray-700 mb-3">{PERMISSIONS_TEXT.PERMISSIONS_TITLE}</h2>
      ...
      <div className="divide-y border-2 border-gray-300 rounded-2xl">
        <Suspense fallback={<p className="text-xs text-gray-400 p-3">{PERMISSIONS_TEXT.LOADING}</p>}>
          {permissions.length === 0 ? (
            <p className="text-xs text-gray-400 p-3">{PERMISSIONS_TEXT.NO_PERMISSIONS_YET}</p>
          ) : (
