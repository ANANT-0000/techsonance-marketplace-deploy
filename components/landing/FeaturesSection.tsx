...
function renderVisual(
    ...
    return (
      <div className="rounded-[1.75rem] border border-landing-border bg-landing-surface p-1 shadow-xl shadow-landing-text/5 overflow-hidden">
        ...
        <div className="p-4 bg-landing-background relative h-72">
          {/* Hero */}
          <div className="w-full h-16 bg-landing-primary-soft rounded-lg flex items-center justify-center text-landing-primary font-bold text-xs uppercase tracking-widest border border-landing-primary/20">SUMMER COLLECTION</div>
          {/* Nav */}
          <div className="flex justify-center gap-6 mt-4 text-[10px] text-landing-muted uppercase font-semibold tracking-wider">
            <span className="text-landing-text">Home</span><span>Shop</span><span>Deals</span><span>About</span>
          </div>
          {/* Products */}
          ...
          <div className="absolute -bottom-4 -right-2 bg-landing-surface border border-landing-border shadow-2xl rounded-xl p-3 w-40 transform rotate-2">
            <div className="flex items-center gap-2 mb-1">
               ...
               <div className="text-[9px] font-bold text-landing-muted uppercase tracking-wider">Inventory</div>
            </div>
            <div className="text-sm font-bold text-landing-text">98% In Stock</div>
          </div>
          <div className="absolute -left-6 top-16 bg-landing-surface border border-landing-border shadow-2xl rounded-xl p-3 w-40 transform -rotate-3">
            <div className="text-[9px] font-bold text-landing-muted uppercase tracking-wider mb-1">Today's Orders</div>
    ...
    return (
      <div className="rounded-[1.75rem] border border-landing-border bg-landing-surface p-5 shadow-xl shadow-landing-text/5">
        <div className="flex justify-between items-center mb-4">
           <div className="font-semibold text-landing-text text-sm">Product Management</div>
           <div className="bg-landing-primary text-landing-on-primary text-[10px] font-semibold px-2.5 py-1 rounded-md">Add Product</div>
        </div>
        <div className="border border-landing-border rounded-xl overflow-hidden bg-landing-background shadow-sm">
          <table className="w-full text-left text-[11px]">
             ...
             <tbody className="divide-y divide-landing-border text-landing-text">
               {[
                 { name: "Modern Sofa", sku: "SOF-001", stock: "12", status: "Active", tone: "success" },
                 { name: "Oak Table", sku: "TAB-092", stock: "0", status: "Empty", tone: "muted" },
                 { name: "Accent Chair", sku: "CHR-044", stock: "48", status: "Active", tone: "success" },
                 { name: "Pendant Light", sku: "LIG-112", stock: "105", status: "Active", tone: "success" },
               ].map(row => (
                 <tr key={row.sku}>
                   ...
                   <td className="p-3 text-center">
                     <span className={`px-2 py-1 rounded-full text-[9px] font-semibold ${row.tone === 'success' ? 'bg-landing-success/10 text-landing-success border border-landing-success/20' : 'bg-landing-muted/10 text-landing-muted border border-landing-border'}`}>{row.status}</span>
                   </td>
                 </tr>
               ))}
             </tbody>
    ...
    return (
      <div className="rounded-[1.75rem] border border-landing-border bg-landing-surface p-5 shadow-xl shadow-landing-text/5 flex flex-col sm:flex-row gap-4">
         <div className="flex-1 border border-landing-border bg-landing-background rounded-2xl p-5 relative overflow-hidden">
           <div className="font-semibold text-landing-text text-sm mb-5">Order #9820</div>
           <div className="space-y-5 relative before:absolute before:inset-y-3 before:left-[11px] before:w-px before:bg-landing-border">
             {[
               ...
               <div key={i} className="flex gap-4 relative z-10">
                 <div className={`w-6 h-6 shrink-0 rounded-full flex items-center justify-center border-2 ${s.done ? 'bg-landing-primary border-landing-primary text-white shadow-[0_0_10px_rgba(var(--primary),0.3)]' : 'bg-landing-background border-landing-border'}`}>
                   {s.done && <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                 </div>
                 <div>
                   <div className={`text-sm font-semibold ${s.done ? 'text-landing-text' : 'text-landing-muted'}`}>{s.step}</div>
                   <div className="text-[10px] text-landing-muted mt-0.5">{s.time}</div>
                 </div>
               </div>
             ))}
           </div>
         ...
         <div className="flex-1 flex flex-col gap-4">
            <div className="bg-landing-primary-soft border border-landing-primary/20 rounded-2xl p-5 flex-1 flex flex-col justify-center items-center text-center shadow-inner">
              <div className="text-[10px] font-bold text-landing-primary uppercase tracking-wider mb-2">Logistics</div>
              <div className="font-bold text-landing-text text-lg">Shiprocket</div>
              <div className="text-xs text-landing-primary mt-1">AWB: 9812739123</div>
            </div>
            <div className="bg-landing-background border border-landing-border rounded-2xl p-5 flex-1 flex flex-col justify-center items-center text-center shadow-sm">
               <div className="text-[10px] font-bold text-landing-muted uppercase tracking-wider mb-2">Payment</div>
               <div className="font-bold text-landing-text text-lg">Razorpay</div>
               <div className="text-xs font-semibold text-landing-success mt-1">₹4,299 Paid</div>
            </div>
         </div>
      </div>
    );
  ...
  if (feature.visual.type === "marketing") {
    return (
      <div className="rounded-[1.75rem] border border-landing-border bg-landing-surface p-5 shadow-xl shadow-landing-text/5">
        <div className="font-semibold text-landing-text text-sm mb-4">Marketing Dashboard</div>
        <div className="grid grid-cols-2 gap-3">
           <div className="bg-landing-primary-soft border border-landing-primary/20 rounded-xl p-4 relative overflow-hidden">
             <div className="text-[10px] uppercase font-bold text-landing-primary tracking-wider mb-1 relative z-10">Active Campaign</div>
             <div className="text-lg font-bold text-landing-text relative z-10">Summer Sale</div>
             <div className="text-xs font-semibold text-landing-success mt-2 relative z-10">↑ 24% Conversion</div>
           ...
           <div className="bg-landing-background border border-landing-border rounded-xl p-4">
             <div className="text-[10px] uppercase font-bold text-landing-muted tracking-wider mb-1">Coupon Code</div>
             <div className="text-sm font-mono font-bold text-landing-text border border-dashed border-landing-primary bg-landing-surface inline-block px-2.5 py-1 rounded mt-1">SUMMER20</div>
           </div>
        </div>
        <div className="mt-3 bg-landing-background border border-landing-border rounded-xl p-4">
           <div className="text-[10px] uppercase font-bold text-landing-muted tracking-wider mb-3">Campaign Revenue</div>
           <div className="flex h-20 items-end gap-1.5">
              {[20, 35, 25, 45, 60, 50, 80, 75, 95].map((h, i) => (
                <div key={i} className={`flex-1 rounded-t-md transition-all duration-500 hover:opacity-80 ${i === 8 ? 'bg-landing-primary' : 'bg-landing-primary/30'}`} style={{height: `${h}%`}}></div>
              ))}
           </div>
        </div>
      </div>
  ...
  return (
      <div className="rounded-[1.75rem] border border-landing-border bg-landing-surface p-5 shadow-xl shadow-landing-text/5">
        <div className="flex justify-between items-center mb-5">
          <div className="font-semibold text-landing-text text-sm">Business Insights</div>
          <div className="bg-landing-background border border-landing-border rounded-md text-[10px] font-semibold text-landing-muted px-2 py-1">Last 30 Days</div>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-5">
           <div className="bg-landing-background border border-landing-border rounded-xl p-4 shadow-sm">
             <div className="text-[10px] uppercase font-bold text-landing-muted tracking-wider mb-1">Total Sales</div>
             <div className="text-2xl font-bold text-landing-text">₹1.24L</div>
             <div className="text-[10px] font-semibold text-landing-success mt-1">↑ 12% vs last month</div>
           </div>
           <div className="bg-landing-background border border-landing-border rounded-xl p-4 shadow-sm">
             <div className="text-[10px] uppercase font-bold text-landing-muted tracking-wider mb-1">Total Orders</div>
             <div className="text-2xl font-bold text-landing-text">842</div>
             <div className="text-[10px] font-semibold text-landing-success mt-1">↑ 8% vs last month</div>
           </div>
        </div>
        <div className="bg-landing-background border border-landing-border rounded-xl p-4 shadow-sm">
           <div className="text-[10px] uppercase font-bold text-landing-muted tracking-wider mb-4">Top Products</div>
             ...
             {[
               ...
               <div key={p.name}>
                 <div className="flex justify-between text-[11px] mb-1.5">
                   <span className="font-semibold text-landing-text">{p.name}</span>
                 ...
                 <div className="w-full bg-landing-surface border border-landing-border rounded-full h-2 overflow-hidden">
                   <div className="bg-landing-primary h-full rounded-full" style={{width: p.fill}}></div>
                 </div>
               </div>
             ))}
           </div>
                  ...
                  >
                    ...
                    <ul className="mt-8 space-y-3">
                      {feature.checklist.map((item, i) => {
                        ...
                        const key = typeof item === 'object' && item !== null && 'id' in item ? (item as any).id : item;
                        return (
                    ...
                    {index === 0 && (
                      <div className="mt-8 flex flex-wrap items-center gap-4">
                        <a href="/auth/vendorRegister" className="inline-flex items-center justify-center rounded-full bg-landing-primary px-5 py-2.5 text-sm font-semibold text-landing-on-primary transition-colors hover:bg-landing-primary-hover">
                          Launch Your Store
                        </a>
                        <a href="#demo" className="inline-flex items-center justify-center rounded-full border border-landing-border bg-landing-surface px-5 py-2.5 text-sm font-semibold text-landing-text transition-colors hover:border-landing-primary hover:bg-landing-primary-soft hover:text-landing-primary">
                          View Demo Store
                        </a>
                      </div>
                    )}
