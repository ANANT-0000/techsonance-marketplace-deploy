...
export default function CampaignForm({ existingData }: { existingData?: any }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white light:bg-gray-800 rounded-xl shadow-sm border border-gray-100 light:border-gray-700 p-6">
      
      {/* Basic Info */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold border-b pb-2 light:border-gray-700">Basic Information</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex flex-col space-y-1.5">
            <label className="text-sm font-medium text-gray-700 light:text-gray-300">Campaign Name</label>
          ...
          <div className="flex flex-col justify-center mt-6">
            <label className="flex items-center cursor-pointer">
              ...
              <span className="ml-2 text-sm text-gray-700 light:text-gray-300">Auto-apply at checkout (No coupon code needed)</span>
            </label>
          </div>
        </div>
        ...
        {!formData.autoApply && (
          <div className="flex flex-col space-y-1.5">
            <label className="text-sm font-medium text-gray-700 light:text-gray-300">Coupon Code</label>
        ...
        <div className="flex flex-col space-y-1.5">
            <label className="text-sm font-medium text-gray-700 light:text-gray-300">Description</label>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold border-b pb-2 light:border-gray-700">Discount Configuration</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex flex-col space-y-1.5">
  <label className="text-sm font-medium text-gray-700 light:text-gray-300">
    Discount Type
  </label>
  <Select 
  ...
  >
    <SelectTrigger className="w-full light:bg-gray-900 light:border-gray-600">
    ...
    <SelectContent>
      ...
      <SelectItem value="bogo">Buy 1 Get 1 (BOGO)</SelectItem>
          ...
          <div className="flex flex-col space-y-1.5">
             <label className="text-sm font-medium text-gray-700 light:text-gray-300">Discount Value</label>
          ...
          {formData.discountType === 'percentage' && (
            <div className="flex flex-col space-y-1.5">
               <label className="text-sm font-medium text-gray-700 light:text-gray-300">Max Discount Cap (₹)</label>
      ...
      {/* Rules & Constraints */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold border-b pb-2 light:border-gray-700">Rules & Validity</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           <div className="flex flex-col space-y-1.5">
             <label className="text-sm font-medium text-gray-700 light:text-gray-300">Valid From</label>
             <Input name="validFrom" type="datetime-local" value={formData.validFrom} onChange={handleChange} required />
           </div>
           
           <div className="flex flex-col space-y-1.5">
             <label className="text-sm font-medium text-gray-700 light:text-gray-300">Valid To</label>
             <Input name="validTo" type="datetime-local" value={formData.validTo} onChange={handleChange} required />
           </div>
           
           <div className="flex flex-col space-y-1.5">
             <label className="text-sm font-medium text-gray-700 light:text-gray-300">Minimum Cart Value (₹)</label>
             <Input name="minCartValue" type="number" value={formData.minCartValue} onChange={handleChange} placeholder="e.g. 999" />
           </div>
           
           <div className="flex flex-col space-y-1.5">
             <label className="text-sm font-medium text-gray-700 light:text-gray-300">Total Global Usages Allowed</label>
